...
- Provides a full web console built on FastAPI ([`webui/`](./webui/README.md)): **chat + experiment lab + training console** in three pages,
...
### 3' (Optional) Web Console

```bash
# The project's full web console: chat / lab (/lab) / training console (/train); no build step, no external deps
python webui/server.py                    # http://localhost:7860 by default
python webui/server.py --no-load          # start without loading a model; pick one in the UI
python webui/server.py --config configs/sft_moe.yaml \
    --weight test/out/full_sft_moe_768_moe.pth --device cuda:0 --port 7860
```

- `/` Chat: streaming output, collapsible thinking chain, multi-turn Tool Call, parameter panel, live resource meters,
  plus the loaded weight's **full architecture detail** and **which training run produced it**.
- `/lab` Lab: the whole pipeline from tokenizer to deployment, page by page (pretrain / SFT / RL / architecture sweep /
  standard eval), including MoE gate health, per-expert load heatmaps, per-layer activations, and reward decomposition.
- `/train` Training console: the form is generated by the backend `/api/catalog`; it shows the *actually effective*
  defaults and the exact argv that will run, and charts live metrics parsed from training stdout.

See [`webui/README.md`](./webui/README.md) for details.

The older Streamlit minimal chat page (it only reads `transformers`-format directories and swallows added tokens such as
`<tool_call>`) is still available:
...
- The training process automatically saves complete checkpoints (model, optimizer, training progress, etc.) in the `./test/checkpoints/` directory
...
> After training, `test/out/pretrain_*.pth` will be produced as output weights (where `*` is the model dimension, default `768`)
...
> After training, `test/out/full_sft_*.pth` will be produced as output weights (where `full` indicates full-parameter fine-tuning)

#### 2.3 Test Trained Model (Optional)

Ensure the model `*.pth` files to be tested are located in the `./test/out/` directory; you can also go directly to [here](https://www.modelscope.cn/models/gongjy/minimind-3-pytorch/files) to download my pre-trained `*.pth` weights.
...
python train_agent.py --rollout_engine sglang --sglang_base_url http://localhost:8998 --sglang_shared_path ./ckpt_mm --data_path ../dataset/rl/agent_rl_math.jsonl --use_wandb
```
...
* [`webui/server.py`](./webui/server.py) is a FastAPI backend serving three pages; the frontend is **plain HTML/CSS/JS** —
  no build step, no npm, no CDN, so it looks identical on an air-gapped machine and on a dev box.
* `/` **Chat**: SSE streaming, collapsible thinking chain, multi-turn Tool Call, parameter panel, live resource meters,
  plus the loaded weight's full architecture detail and which training run produced it.
* `/lab` **Lab** (10 pages): the whole pipeline from tokenizer to deployment, page by page (pretrain / SFT / RL /
  architecture sweep / standard eval), including MoE gate health, per-expert load heatmaps, per-layer activations,
  reward decomposition, and each algorithm's own metrics with a coverage matrix.
* `/train` **Training console**: the form is generated by the backend `/api/catalog`; it shows the actually effective
  defaults and the exact argv that will run, and charts live metrics parsed from training stdout.
* Data is read from [`test/`](./test/README.md) by default (`MINIMIND_ARTIFACT_ROOT` overrides it); see
  [`webui/README.md`](./webui/README.md) for per-page details, the API list, and the metric → page mapping.
...
> The older Streamlit minimal chat page (`scripts/web_demo.py`) is still available, but it only reads
> `transformers`-format directories and decodes with `skip_special_tokens=True` — `<tool_call>` / `<think>` are added
> tokens, so they get swallowed.
